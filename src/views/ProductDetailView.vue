  ...
  <section class="py-10 px-4 max-w-4xl mx-auto">
    <div v-if="product" class="space-y-6">
      ...
      <div class="text-lg">
        ...
        <p><strong>售價：</strong>{{ product.price }}</p>
        <p><strong>簡介：</strong>{{ product.description }}</p>
        <p><strong>煮法建議：</strong>{{ product.cookingMethod }}</p>
        <p>
          <strong>認證：</strong>
          <span v-for="(cert, i) in product.certifications" :key="i"
            class="inline-block bg-green-100 text-green-800 px-2 py-1 rounded mr-2">
            {{ cert }}
          </span>
        </p>
      </div>

      <a v-if="product.buyLink" :href="product.buyLink" target="_blank"
        class="inline-block mt-4 px-6 py-2 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition">
        前往購買
      </a>
    </div>
